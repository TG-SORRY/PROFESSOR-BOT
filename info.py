...
# PyroClient Setup 
API_ID = int(environ['API_ID', "26994329"])
API_HASH = environ['API_HASH', "e5a99882a2415b26cac65064089b9bec"]
BOT_TOKEN = environ['BOT_TOKEN', "8128479667:AAEZJfq8DDldhHsHyOUZxGznCpBT-OBplAU"]
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1834908260 1727779215').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002446904461').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1834908260').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', "-1001533227813")
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://jofewal597:jofewal597@cluster0.9eavxki.mongodb.net/?retryWrites=true&w=majority")
DATABASE_NAME = environ.get('DATABASE_NAME', "jofewal597")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', "-1001780923121"))
